...
class ReplayBuffer():
    def __init__(self, max_size, input_shape, n_actions, seq_len):
        ...
        self.next_states = np.zeros((max_size, *input_shape), dtype=np.float32)
...
class ReplayBuffer_():
    def __init__(self, max_size, input_shape, n_actions):
        ...
        self.mem_cntr = 0
        self.state_memory = np.zeros((self.mem_size, *input_shape), dtype=np.float32) ## needs to be float
        self.new_state_memory = np.zeros((self.mem_size, *input_shape), dtype=np.float32)
        self.action_seq_memory = np.zeros((self.mem_size, *n_actions), dtype=np.float32)
