...
import numpy as np
import copy
from SAC_Self_Attension.Buffer import ReplayBuffer
from SAC_Self_Attension.Networks import ActorNetwork, CriticNetwork

class Agent():
    def __init__(self, input_dims, n_actions, seq_len, attention_heads, env, seed, lr = 0.001,  
            gamma=0.99, max_size=100000, tau=0.005, batch_size=360, reward_scale=1):
        self.gamma = gamma
        self.seed = seed
        self.tau = tau
        ...
        # Networks
        self.actor = ActorNetwork(self.lr, input_dims, n_actions, env, seq_len, attention_heads)
        self.critic_1 = CriticNetwork(self.lr, input_dims, seq_len, n_actions, attention_heads)
        self.critic_2 = CriticNetwork(self.lr, input_dims, seq_len, n_actions, attention_heads)
    ...
    def save_models(self):
        print('.... saving models ....')
        T.save(self.actor.state_dict(), 'models/Actor')
        T.save(self.critic_1.state_dict(), 'models/Critic_1')
        T.save(self.critic_2.state_dict(), 'models/Critic_2')
    ...
    def learn(self):
      ...
      # ACTOR AND ALPHA UPDATE ----------------------------
        ...
        q1_value = self.critic_1.forward(state, actions).view(-1)
        q2_value = self.critic_2.forward(state, actions).view(-1)
        with T.no_grad():
            action_, log_probs_ = self.actor.sample_normal(state_, reparameterize=True)
            log_probs_ = log_probs_.view(-1)
            target_q1_value = self.target_critic_1.forward(state_, action_)
            target_q2_value = self.target_critic_2.forward(state_, action_)
