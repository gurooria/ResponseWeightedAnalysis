...
class Agent():
    def __init__(self, input_dims, n_actions, seq_len, env, seed,attention_heads = 2, lr = 0.0003,  
            gamma=0.99, max_size=50000, tau=0.005, batch_size=120, reward_scale=1):
        self.gamma = gamma
        self.seed = seed
        self.tau = tau
        ...
        #(channels, width, height) 2 images = 6 channels
        self.memory = ReplayBuffer(max_size, (6, input_dims, input_dims), n_actions, seq_len)
        ...
        # Networks -> the channels are indicated in the network (6)
        self.actor = ActorNetwork(self.lr, input_dims, n_actions, env, seq_len, attention_heads, self.hidden_dims)
        self.critic_1 = CriticNetwork(self.lr, input_dims, seq_len, n_actions, attention_heads, self.hidden_dims)
        self.critic_2 = CriticNetwork(self.lr, input_dims, seq_len, n_actions, attention_heads, self.hidden_dims)
    ...
    def save_models(self, subpolicy, epoch):
        print('.... saving models ....')
        T.save(self.actor.state_dict(), f'Models/models_sub_{subpolicy}/Actor')
        T.save(self.critic_1.state_dict(), f'Models/models_sub_{subpolicy}/Critic_1')
        T.save(self.critic_2.state_dict(), f'Models/models_sub_{subpolicy}/Critic_2')
        T.save(self.actor.state_dict(), f'Models/models_sub_{subpolicy}/Epoch_weights/Actor_{epoch}')
    ...
    def learn(self):
      ...
      # ACTOR AND ALPHA UPDATE ----------------------------
        ...
        # Delete temporary tensors that are no longer needed
        del state, action_seq, reward, new_state, done, state_, seq_actions_, seq_actions, last_action
        del q1_new_policy, q2_new_policy, q1_value, q2_value, target_q1_value, target_q2_value, target_q_value, final_target
